...
#### Running richness calculations over all the years
...
######### put all together and save
list_csv_files <- list.files(path = "5yr_summary_2.5distance/", pattern="*.csv")
#dat <- readr::read_csv(paste("5yr_summary/", list_csv_files, sep=""), id = "file_name")
...
for(i in 1:length(list_csv_files)) {                              # Head of for-loop
  assign(names[i],                                   # Read and store data frames
         read.csv(paste("5yr_summary_2.5distance/", list_csv_files[i], sep="")))
}
...
dat <- bind_rows(r1c1, r1c2, r1c3, r1c4, 
                 ...
                 r4c1, r4c2, r4c3, r4c4)
#save all the summaries as a csv
write.csv(dat, "global_richness_summary_2.5distance.csv", row.names=FALSE)

# find top 500 cells
top_cells <- dat %>% slice_max(total_SR, n=500)
write.csv(top_cells, "top_500_cells_2.5distance.csv", row.names=FALSE)
...
foreach (j=1:length(names)) %dopar%{
  ...
  for (i in 1:length(years)) {
    dat <- read.table(paste("/Volumes/Backup/eBird/eBird_", years[i], "_data/custom_bbox/", names[j], "_", years[i], "_filt.txt", sep=""), 
                      header=TRUE) # load data
    dat %>% filter(effort_distance_km <= 1)
    dat$SCIENTIFIC.NAME <- as.character(dat$SCIENTIFIC.NAME)
    dat$OBSERVATION.DATE <- as.character(dat$OBSERVATION.DATE)
    dat$OBSERVER.ID <- as.character(dat$OBSERVER.ID)
    dat$SAMPLING.EVENT.IDENTIFIER <- as.character(dat$SAMPLING.EVENT.IDENTIFIER)
...
######### put all together and save
list_csv_files <- list.files(path = "5yr_summary_1kmdistance/", pattern="*.csv")
#dat <- readr::read_csv(paste("5yr_summary/", list_csv_files, sep=""), id = "file_name")
...
for(i in 1:length(list_csv_files)) {                              # Head of for-loop
  assign(names[i],                                   # Read and store data frames
         read.csv(paste("5yr_summary_1kmdistance/", list_csv_files[i], sep="")))
}
...
dat <- bind_rows(r1c1, r1c2, r1c3, r1c4, 
                 ...
                 r4c1, r4c2, r4c3, r4c4)
#save all the summaries as a csv
write.csv(dat, "global_richness_summary_1kmdistance.csv", row.names=FALSE)

# find top 500 cells
top_cells <- dat %>% slice_max(total_SR, n=500)
write.csv(top_cells, "top_500_cells_1kmdistance.csv", row.names=FALSE)
